...
class DataCollection(object):
    ...
    def queryDeviceDataHistory(self, qddhInDTO, accessToken):
        ...
        # authUrl = RestConstant.QUERY_DEVICE_DATA_HISTORY + qddhInDTO['deviceId'] + "&gatewayId=" + qddhInDTO[
        #     'gatewayId']
        authUrl = RestConstant.QUERY_DEVICE_DATA_HISTORY
        for key in qddhInDTO.keys():
            if qddhInDTO[key] != None:
                # print("+++++++++++key="+key+"  keyvalue="+qddhInDTO[key])
                authUrl += "&" + key + "=" + str(qddhInDTO[key])
    ...
    def queryDeviceDesiredHistory(self, qddhInDTO, accessToken):
        qddhInDTO = DictUtil.dto2dict(qddhInDTO)
        authUrl = RestConstant.QUERY_DEVICE_DESIRED_HISTORY + qddhInDTO['deviceId'] + "&gatewayId=" + qddhInDTO['gatewayId']
        for key in qddhInDTO.keys():
            if qddhInDTO[key] != None:
